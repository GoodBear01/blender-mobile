...
echo "== Xcode is compiling the full editor (libblender target), then signing the app =="
echo "The first build can take hours."
xcodebuild \
  -project "$PROJ" \
...
if [[ ! -f "$APP/Frameworks/libblender.dylib" && "${STUB:-}" != "1" ]]; then
  echo "The .app still has no libblender.dylib. Check the libblender target in the Xcode report." >&2
  exit 1
fi
