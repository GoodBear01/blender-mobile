...
# Phone builds keep Cycles on even if an older userpref dropped it.
_android_force_addons = (
    ("cycles",)
    if (
        _os.environ.get("BLENDER_ANDROID")
        or _os.environ.get("BLENDER_IOS")
        or _os.environ.get("BLENDER_MOBILE")
    )
    else ()
)
...
def _initialize_once():
    ...
    for module_name in _android_force_addons:
        enable(
            module_name,
            refresh_handled=True,
            default_set=True,
