...
class WM_MT_splash_quick_setup(Menu):
    ...
    def draw_android(self, context):
        layout = self.layout
        prefs = context.preferences
        wm = context.window_manager

        layout.label(text="Phone Setup")
        layout.separator()
        col = layout.column(align=True)
        col.scale_y = 1.45
        ...
        if label == "Presets":
            label = "Blender Dark"
        col.menu("USERPREF_MT_interface_theme_presets", text=label)

        kc = wm.keyconfigs.active
        text = bpy.path.display_name(kc.name) or "Blender"
        col.menu("USERPREF_MT_keyconfigs", text=text)

        col.separator()
        col.prop(prefs.view, "ui_scale", text="UI Size", slider=True)
        size_row = col.row(align=True)
        smaller = size_row.operator("wm.android_ui_scale", text="Smaller")
        smaller.delta = -0.10
        smaller.value = 0.0
        larger = size_row.operator("wm.android_ui_scale", text="Larger")
        larger.delta = 0.10
        larger.value = 0.0
        if hasattr(prefs.view, "gizmo_size_navigate_v3d"):
            col.prop(prefs.view, "gizmo_size_navigate_v3d", text="Nav Widget")

        layout.separator()
        extra = layout.column(align=True)
        extra.scale_y = 1.35
        extra.operator_context = 'INVOKE_DEFAULT'
        extra.operator("wm.android_mobile_settings", text="More Phone Settings", icon='PREFERENCES')

        layout.separator()
        start = layout.column(align=True)
        start.scale_y = 1.7
        start.operator_context = 'EXEC_DEFAULT'
        start.operator("wm.android_start", text="Start Phone Layout", icon='PLAY')
        layout.separator()
        layout.label(text="Drag to orbit  ·  two fingers to pan")
        layout.label(text="Pinch to zoom  ·  right-click for menu")
...
class WM_MT_splash(Menu):
    ...
    def draw_android(self, context):
        try:
            self._draw_android_body(context)
        except Exception as exc:
            print("android_phone: splash draw failed", exc)
            layout = self.layout
            layout.label(text="Blender for Android")
            props = layout.operator("wm.read_homefile", text="New Scene", icon='FILE_NEW')
            props.app_template = ""

    def _draw_android_body(self, context):
        layout = self.layout
        layout.operator_context = 'INVOKE_DEFAULT'

        phone = layout.column(align=True)
        phone.scale_y = 1.55
        phone.operator("wm.android_mobile_settings", text="Phone Settings", icon='PREFERENCES')
        phone.operator_context = 'EXEC_DEFAULT'
        phone.operator("wm.android_apply_layout", text="Force Phone Layout", icon='FULLSCREEN_ENTER')

        layout.separator()
        hero = layout.column(align=True)
        hero.scale_y = 1.7
        hero.operator_context = 'EXEC_DEFAULT'
        props = hero.operator("wm.read_homefile", text="New Scene", icon='FILE_NEW')
        props.app_template = ""

        layout.separator()
        open_col = layout.column(align=True)
        open_col.scale_y = 1.45
        open_col.operator_context = 'EXEC_DEFAULT'
        open_col.operator("wm.android_open_file", text="Open from Phone", icon='FILE_FOLDER')
        open_col.operator("wm.android_browse_storage", text="Browse Phone Files", icon='DISK_DRIVE')
        open_col.operator("wm.android_grant_files", text="Allow File Access", icon='UNLOCKED')
        open_col.operator("wm.recover_last_session", text="Recover Last Session", icon='RECOVER_LAST')

        rec = layout.column()
        rec_title = rec.row()
        found_recent = rec.template_recent_files(rows=3)
        if found_recent:
            rec_title.label(text="Recent Files")

        layout.separator()
        more = layout.column(align=True)
        more.scale_y = 1.2
        more.operator_context = 'EXEC_DEFAULT'
        more.menu("TOPBAR_MT_file_new", text="More Templates")
        layout.separator()
        layout.label(text="Drag to orbit  ·  two fingers to pan")
        layout.label(text="Pinch to zoom  ·  right-click for menu")
        layout.label(text="Tap the picture to close")
