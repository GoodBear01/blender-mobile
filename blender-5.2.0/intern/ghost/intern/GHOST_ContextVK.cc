...
#else /* X11/WAYLAND/SDL/Android/iOS. */
...
struct GHOST_InstanceVK {
                     ...
                     blender::Span<const char *> optional_device_extensions)
  {
    ...
    device_features.fragmentStoresAndAtomics = device.features.features.fragmentStoresAndAtomics;
    ...
    device_features.imageCubeArray = device.features.features.imageCubeArray;
    ...
    vulkan_11_features.shaderDrawParameters =
        device.features_11.shaderDrawParameters ||
        device.extensions.is_supported(VK_KHR_SHADER_DRAW_PARAMETERS_EXTENSION_NAME) ||
        device.extensions.is_enabled(VK_KHR_SHADER_DRAW_PARAMETERS_EXTENSION_NAME);
#else
    ...
    vulkan_12_features.timelineSemaphore = device.features_12.timelineSemaphore;
#ifdef __ANDROID__
    /* 1.1 HALs often omit Vulkan12Features. Enable the KHR equivalents instead. */
    ...
    const uint32_t api = device.properties.properties.apiVersion;
    const bool vulkan_12 = VK_API_VERSION_MAJOR(api) > 1 || VK_API_VERSION_MINOR(api) >= 2;
    if (vulkan_12) {
      feature_struct_ptr.push_back(&vulkan_12_features);
    }
    if (!vulkan_12_features.timelineSemaphore &&
        (device.extensions.is_supported(VK_KHR_TIMELINE_SEMAPHORE_EXTENSION_NAME) ||
         device.extensions.is_enabled(VK_KHR_TIMELINE_SEMAPHORE_EXTENSION_NAME)))
    {
      feature_struct_ptr.push_back(&timeline_features);
    }
    if (!vulkan_12_features.bufferDeviceAddress &&
        (device.extensions.is_supported(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME) ||
         device.extensions.is_enabled(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME)))
    {
      feature_struct_ptr.push_back(&bda_features);
    }
#else
    feature_struct_ptr.push_back(&vulkan_12_features);
#endif
    ...
    /* Enable provoking vertex when the extension is present (often missing on Android). */
    ...
    if (device.extensions.is_supported(VK_EXT_PROVOKING_VERTEX_EXTENSION_NAME) ||
        device.extensions.is_enabled(VK_EXT_PROVOKING_VERTEX_EXTENSION_NAME))
    {
...
#ifdef WITH_GHOST_SDL
  if (surface_ == VK_NULL_HANDLE && sdl_window_ != nullptr) {
...
#ifdef WITH_GHOST_SDL
      else if (acquire_result == VK_ERROR_SURFACE_LOST_KHR) {
...
#ifdef __ANDROID__
  /* Rotation often leaves currentExtent at the portrait size, UINT32_MAX, or
   * swapped vs the SDL window. Blender lays out in window space, so the
   * swapchain must match the window pixel size with identity pre-transform. */
  int android_w = 0;
  int android_h = 0;
  if (sdl_window_ != nullptr) {
    SDL_GetWindowSizeInPixels(sdl_window_, &android_w, &android_h);
  }
  const bool android_extent_unusable = (render_extent_.width == 0 || render_extent_.height == 0 ||
                                        render_extent_.width == UINT32_MAX ||
                                        render_extent_.height == UINT32_MAX);
  if (android_w > 0 && android_h > 0 &&
      (android_extent_unusable || int(render_extent_.width) != android_w ||
       int(render_extent_.height) != android_h))
  {
    render_extent_.width = uint32_t(android_w);
    render_extent_.height = uint32_t(android_h);
    ...
    if (capabilities.maxImageExtent.width != 0 &&
        capabilities.maxImageExtent.width < render_extent_.width)
    {
      render_extent_.width = capabilities.maxImageExtent.width;
    }
    if (capabilities.maxImageExtent.height != 0 &&
        capabilities.maxImageExtent.height < render_extent_.height)
    {
...
#endif
#ifdef WITH_GHOST_SDL
    if ((width == 0 || height == 0) && sdl_window_ != nullptr) {
...
#ifdef __ANDROID__
  /* Blender renders in window space. Identity keeps UI/touch aligned after rotate. */
...
#ifdef WITH_GHOST_SDL
GHOST_TSuccess GHOST_ContextVK::destroyWindowSurface()
{
  if (!vulkan_instance.has_value()) {
    return GHOST_kFailure;
  }
...
GHOST_TSuccess GHOST_ContextVK::recreateWindowSurface()
{
  if (sdl_window_ == nullptr || !vulkan_instance.has_value()) {
    return GHOST_kFailure;
  }
  GHOST_InstanceVK &instance_vk = vulkan_instance.value();
  if (surface_ == VK_NULL_HANDLE) {
  ...
  if (!instance_vk.device.has_value()) {
    return GHOST_kFailure;
  }
  const bool use_hdr_swapchain = hdr_info_ &&
                                 (hdr_info_->wide_gamut_enabled || hdr_info_->hdr_enabled) &&
                                 instance_vk.device->use_vk_ext_swapchain_colorspace;
  return recreateSwapchain(use_hdr_swapchain);
}

GHOST_TSuccess GHOST_ContextVK::updateDrawingContext()
{
  if (sdl_window_ == nullptr) {
    return GHOST_kFailure;
  }
  destroyWindowSurface();
  const SDL_WindowFlags flags = SDL_GetWindowFlags(sdl_window_);
...
#  endif
  ...
  if (!vulkan_instance->device.has_value()) {
    ...
    optional_device_extensions.append(VK_KHR_SHADER_DRAW_PARAMETERS_EXTENSION_NAME);
#endif
