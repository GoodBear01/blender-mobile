...
static void rna_def_function_funcs(FILE *f, StructDefRNA *dsrna, FunctionDefRNA *dfunc)
{
  ...
  for (; dparm; dparm = dparm->next) {
    ...
    if (dparm->next) {
      /* Do not bake host sizeof(PointerRNA). Android WITH_ASSERT_RELEASE
       * adds Vector::debug_size_ and the NDK type is larger than the host
       * makesrna binary used to generate these files. */
      if (type == PROP_POINTER && (flag_parameter & PARM_RNAPTR) && (flag & PROP_THICK_WRAP)) {
        fprintf(f,
                "\t_data += (sizeof(PointerRNA) + sizeof(void *) - 1) & ~(sizeof(void *) - 1);\n");
      }
      else {
