...
#ifdef BLENDER_MOBILE
  /* A tap can emit both CLICK and RELEASE; opening twice freezes the phone. */
  static double last_android_file_exec = 0.0;
  const double now = BLI_time_now_seconds();
  if (now - last_android_file_exec < 0.85) {
    return OPERATOR_CANCELLED;
  }
  last_android_file_exec = now;
#endif
