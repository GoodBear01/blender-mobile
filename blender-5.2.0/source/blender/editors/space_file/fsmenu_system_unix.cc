...
void fsmenu_read_system(FSMenu *fsmenu, int read_bookmarks)
{
  ...
  const char *storage = BLI_getenv("BLENDER_ANDROID_STORAGE");
  if (storage == nullptr || storage[0] == '\0') {
    storage = BLI_getenv("BLENDER_IOS_DOCUMENTS");
  }
  if (storage == nullptr || storage[0] == '\0') {
#ifdef BLENDER_IOS
    storage = BLI_dir_home();
#else
    storage = "/storage/emulated/0";
#endif
  }
  if (read_bookmarks) {
    auto add_public = [&](const char *env, const char *fallback, const char *label, int icon) {
      const char *path = BLI_getenv(env);
      char joined[FILE_MAXDIR];
      if (path == nullptr || path[0] == '\0') {
        BLI_path_join(joined, sizeof(joined), storage, fallback);
        path = joined;
      }
      fsmenu_insert_entry(fsmenu, FS_CATEGORY_SYSTEM_BOOKMARKS, path, label, icon, FS_INSERT_LAST);
    };
    fsmenu_insert_entry(
        fsmenu, FS_CATEGORY_SYSTEM, storage, N_("Phone Storage"), ICON_DISK_DRIVE, FS_INSERT_LAST);
    add_public("BLENDER_ANDROID_DOWNLOADS", "Download", N_("Download"), ICON_IMPORT);
    add_public("BLENDER_ANDROID_BLENDS", "Download/Blender", N_("Blender Files"), ICON_FILE_FOLDER);
    add_public("BLENDER_ANDROID_DOCUMENTS", "Documents", N_("Documents"), ICON_DOCUMENTS);
    add_public("BLENDER_ANDROID_PICTURES", "Pictures", N_("Pictures"), ICON_FILE_IMAGE);
    add_public("BLENDER_ANDROID_DCIM", "DCIM", N_("Camera"), ICON_FILE_IMAGE);
    add_public("BLENDER_ANDROID_MOVIES", "Movies", N_("Movies"), ICON_FILE_MOVIE);
    add_public("BLENDER_ANDROID_MUSIC", "Music", N_("Music"), ICON_FILE_SOUND);
  }
  return;
#endif
