...
static Vector<StringRefNull> missing_capabilities_get(VkPhysicalDevice vk_physical_device)
{
  ...
  /* Check device features. Also query KHR structs: Android 1.1 devices often
   * expose timeline/BDA as extensions, not as Vulkan 1.2 core bits. */
  VkPhysicalDeviceVulkan12Features features_12 = {
      VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_2_FEATURES};
  VkPhysicalDeviceVulkan11Features features_11 = {
      VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_1_FEATURES, &features_12};
  VkPhysicalDeviceTimelineSemaphoreFeatures timeline_features = {
      VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_TIMELINE_SEMAPHORE_FEATURES, &features_11};
  VkPhysicalDeviceBufferDeviceAddressFeatures bda_features = {
      VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_BUFFER_DEVICE_ADDRESS_FEATURES, &timeline_features};
...
#  endif
  ...
  if (features_12.timelineSemaphore == VK_FALSE && timeline_features.timelineSemaphore == VK_FALSE)
  {
    missing_capabilities.append("timeline semaphores");
  }
  if (features_12.bufferDeviceAddress == VK_FALSE && bda_features.bufferDeviceAddress == VK_FALSE)
  {
...
#ifdef BLENDER_MOBILE
  /* Vulkan 1.3 promotes dynamic rendering; some HALs stop advertising the KHR name. */
  const bool vulkan_13 = VK_API_VERSION_MAJOR(vk_properties.apiVersion) > 1 ||
                         VK_API_VERSION_MINOR(vk_properties.apiVersion) >= 3;
  if (!extensions.contains(VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME) && !vulkan_13) {
    missing_capabilities.append(VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME);
  }
  if (features_11.shaderDrawParameters == VK_FALSE &&
      !extensions.contains(VK_KHR_SHADER_DRAW_PARAMETERS_EXTENSION_NAME))
  {
    missing_capabilities.append("shader draw parameters");
  }
#else
  ...
  /* Metal doesn't support provoking vertex. Many Android GPUs omit it too. */
...
{
#ifdef BLENDER_MOBILE
  /* Phone loaders (Android HAL / MoltenVK) are not the desktop Khronos loader. */
  return;
#endif
...
static bool vk_instance_create_for_platform_checks(VkInstance *r_instance)
{
  ...
  *r_instance = VK_NULL_HANDLE;
  ...
  for (const uint32_t api_version : api_versions) {
    ...
    if (vkCreateInstance(&vk_instance_info, nullptr, r_instance) == VK_SUCCESS &&
        *r_instance != VK_NULL_HANDLE)
    {
      return true;
    }
