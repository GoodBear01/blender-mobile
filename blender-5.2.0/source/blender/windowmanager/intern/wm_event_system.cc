...
#ifdef BLENDER_MOBILE
      const int file_display_type = USER_TEMP_SPACE_DISPLAY_FULLSCREEN;
#else
      const int file_display_type = U.filebrowser_display_type;
#endif
      ScrArea *area = ED_screen_temp_space_open(
          C, IFACE_("Blender File View"), SPACE_FILE, file_display_type, true);
      if (!area) {
...
static ARegion *wm_android_popup_region_at(wmWindow *win, const int xy[2])
{
  ...
  return BKE_screen_find_region_xy(screen, RGN_TYPE_ANY, xy);
}

static bool wm_android_region_has_button(ARegion *region, wmWindow *win, const int xy[2])
{
  if (region == nullptr || win->runtime == nullptr || win->runtime->eventstate == nullptr) {
    return false;
  }
  wmEvent event = *win->runtime->eventstate;
  copy_v2_v2_int(event.xy, xy);
  ...
  event.val = KM_PRESS;
  return ui::but_find_mouse_over(region, &event) != nullptr;
}

static ARegion *wm_android_view3d_region_at(wmWindow *win, const int xy[2])
{
  bScreen *screen = WM_window_get_active_screen(win);
  if (screen == nullptr) {
    return nullptr;
  }
  /* Splash, menus, and other popups sit above every editor. */
  if (wm_android_popup_region_at(win, xy) != nullptr) {
    return nullptr;
  }
  ...
  /* Overlapping headers/sidebars share the 3D window rect. Use the
   * same visual hit-test as the event system so taps hit buttons. */
  ARegion *visual = ED_area_find_region_xy_visual(area, RGN_TYPE_ANY, xy);
  if (visual != nullptr && visual->regiontype != RGN_TYPE_WINDOW) {
    return nullptr;
  }
  for (ARegion &region : area->regionbase) {
    ...
    if (wm_android_region_has_button(&region, win, xy)) {
      return nullptr;
    }
  }
  if (visual != nullptr && visual->regiontype == RGN_TYPE_WINDOW && visual->regiondata != nullptr) {
    return visual;
  }
  return nullptr;
}

static void wm_android_orbit_rv3d(RegionView3D *rv3d, const int dx, const int dy)
{
  ...
  if (len_squared_v3v3(zvec_global, m_inv[2]) > 0.001f) {
    ...
    if (dot_v3v3(xaxis, m_inv[0]) < 0.0f) {
      negate_v3(xaxis);
    }
    float fac = angle_normalized_v3v3(zvec_global, m_inv[2]) / float(M_PI);
    fac = fabsf(fac - 0.5f) * 2.0f;
    fac = fac * fac;
  ...
  float quat_x[4];
  float quat_z[4];
  axis_angle_to_quat(quat_x, xaxis, sensitivity * float(-dy));
  mul_qt_qtqt(quat_x, curr, quat_x);
  axis_angle_to_quat_single(quat_z, 'Z', sensitivity * float(dx));
  mul_qt_qtqt(rv3d->viewquat, quat_x, quat_z);
  normalize_qt(rv3d->viewquat);
}

static void wm_android_pan_rv3d(RegionView3D *rv3d, const int dx, const int dy)
{
  if (RV3D_LOCK_FLAGS(rv3d) & RV3D_LOCK_LOCATION) {
    return;
  }
  float dvec[3] = {float(-dx), float(-dy), 0.0f};
  mul_v3_fl(dvec, rv3d->dist * 0.0015f);
  float inv[4];
  invert_qt_qt_normalized(inv, rv3d->viewquat);
  mul_qt_v3(inv, dvec);
  add_v3_v3(rv3d->ofs, dvec);
}

static bool wm_android_gizmo_owns_pointer(ARegion *region)
{
  if (region == nullptr || region->runtime == nullptr || region->runtime->gizmo_map == nullptr) {
    return false;
  }
  wmGizmoMap *gzmap = region->runtime->gizmo_map;
  return wm_gizmomap_highlight_get(gzmap) != nullptr || wm_gizmomap_modal_get(gzmap) != nullptr;
}

static bool wm_android_modal_op_running(wmWindow *win)
{
  for (wmEventHandler &handler : win->runtime->modalhandlers) {
...
static ARegion *wm_android_nav_region(wmWindow *win, const int xy[2])
{
  ARegion *region = wm_android_view3d_region_at(win, xy);
...
static bool wm_android_near_area_edge(wmWindow *win, const int xy[2])
{
  bScreen *screen = WM_window_get_active_screen(win);
  if (screen == nullptr) {
    return false;
  }
  const int pad = int(24.0f * UI_SCALE_FAC);
  rcti screen_rect;
  WM_window_screen_rect_calc(win, &screen_rect);
  for (ScrArea &area : screen->areabase) {
    const rcti &r = area.totrct;
    if (xy[1] < r.ymin - pad || xy[1] > r.ymax + pad || xy[0] < r.xmin - pad ||
        xy[0] > r.xmax + pad)
    {
      continue;
    }
    const bool inner_left = r.xmin > screen_rect.xmin + 2;
    const bool inner_right = r.xmax < screen_rect.xmax - 2;
    const bool inner_bottom = r.ymin > screen_rect.ymin + 2;
    const bool inner_top = r.ymax < screen_rect.ymax - 2;
    if ((inner_left && abs(xy[0] - r.xmin) <= pad && xy[1] >= r.ymin && xy[1] <= r.ymax) ||
        (inner_right && abs(xy[0] - r.xmax) <= pad && xy[1] >= r.ymin && xy[1] <= r.ymax) ||
        (inner_bottom && abs(xy[1] - r.ymin) <= pad && xy[0] >= r.xmin && xy[0] <= r.xmax) ||
        (inner_top && abs(xy[1] - r.ymax) <= pad && xy[0] >= r.xmin && xy[0] <= r.xmax))
    {
      return true;
    }
  }
...
static ARegion *wm_android_region_at(wmWindow *win, const int xy[2])
{
  if (ARegion *popup = wm_android_popup_region_at(win, xy)) {
    return popup;
  }
  bScreen *screen = WM_window_get_active_screen(win);
  if (screen == nullptr) {
  ...
  if (ARegion *visual = ED_area_find_region_xy_visual(area, RGN_TYPE_ANY, xy)) {
    return visual;
  }
  return BKE_area_find_region_xy(area, RGN_TYPE_ANY, xy);
}

static bool wm_android_ui_should_scroll(wmWindow *win, const int xy[2])
{
  return wm_android_view3d_region_at(win, xy) == nullptr;
}

static bool wm_android_ui_hit_is_click_target(wmWindow *win, const int xy[2])
{
  ARegion *region = wm_android_region_at(win, xy);
  if (region == nullptr) {
    return false;
  }
  if (RGN_TYPE_IS_HEADER_ANY(region->regiontype) || region->regiontype == RGN_TYPE_NAV_BAR ||
  ...
  /* uiList rows still drag-scroll after slop; a tap stays a click. */
  if (ui::uilist_find_mouse_over(region, &event) != nullptr) {
    return false;
  }
  return ui::but_find_mouse_over(region, &event) != nullptr;
}

static void wm_android_inject_mousepan(wmWindow *win, const int xy[2], int dx, int dy)
{
  if (dx == 0 && dy == 0) {
    return;
  }
...
static void wm_android_on_button(wmWindow *win, const int type, const short val, const int xy[2])
{
  ...
  if (type == LEFTMOUSE) {
    ...
    /* Real RMB is the context menu. Two-finger pan no longer spoofs RMB. */
    return;
  }
  ...
  if (val == KM_PRESS) {
    ...
    /* Popups sit on top of the 3D view; never orbit through them. */
    if (type == LEFTMOUSE && wm_android_near_area_edge(win, xy)) {
      android_buttons &= ~uint8_t(bit);
    }
    else if (wm_android_ui_should_scroll(win, xy)) {
      android_buttons &= ~uint8_t(bit);
      /* Buttons, headers, and sliders keep the tap. Only empty/list
       * areas may convert a drag into scroll. */
      if (type == LEFTMOUSE && !wm_android_ui_hit_is_click_target(win, xy)) {
        android_ui_hold = true;
      }
    }
  }
...
static void wm_android_apply_pan(wmWindow *win, const int xy[2], int dx, int dy)
{
  ...
  wm_android_pan_rv3d(static_cast<RegionView3D *>(region->regiondata), dx, dy);
  ED_region_tag_redraw(region);
}
...
static void wm_android_navigate_from_move(wmWindow *win, const int xy[2])
{
  if (android_ui_hold) {
    ...
    if (!android_ui_scrolling) {
      const int adx = xy[0] - android_press_xy[0];
      const int ady = xy[1] - android_press_xy[1];
      if (adx * adx + ady * ady < kAndroidUiScrollArmPx * kAndroidUiScrollArmPx) {
        android_last_xy[0] = xy[0];
        android_last_xy[1] = xy[1];
        return;
      }
      android_ui_scrolling = true;
      wmEvent release = *win->runtime->eventstate;
      release.type = LEFTMOUSE;
      release.val = KM_RELEASE;
      copy_v2_v2_int(release.xy, xy);
      wm_event_add_intern(win, &release);
    }
    android_last_xy[0] = xy[0];
    android_last_xy[1] = xy[1];
    wm_android_inject_mousepan(win, xy, dx, dy);
    return;
  }
  if (android_buttons == 0) {
    android_last_xy[0] = xy[0];
  ...
  if (wm_android_modal_op_running(win) || wm_android_near_area_edge(win, xy)) {
    android_last_xy[0] = xy[0];
    android_last_xy[1] = xy[1];
    return;
  }
  ...
  if (!android_nav_armed) {
    const int adx = xy[0] - android_press_xy[0];
    const int ady = xy[1] - android_press_xy[1];
    if (adx * adx + ady * ady < kAndroidOrbitArmPx * kAndroidOrbitArmPx) {
      android_last_xy[0] = xy[0];
      android_last_xy[1] = xy[1];
      return;
    }
    android_nav_armed = true;
    android_last_xy[0] = xy[0];
    android_last_xy[1] = xy[1];
    return;
  }
                             ...
                             const uint64_t event_time_ms)
{
  ...
  switch (type) {
    ...
    case GHOST_kEventTrackpad: {
      ...
      event.val = KM_NOTHING;
#ifdef BLENDER_MOBILE
      if (pd->subtype == GHOST_kTrackpadEventScroll) {
        if (wm_android_ui_should_scroll(win, event.xy)) {
          event.type = MOUSEPAN;
          wm_event_add_trackpad(win, &event, delta[0], delta[1]);
        }
        else {
          /* Two-finger pan in the 3D view only. */
          wm_android_apply_pan(win, event.xy, delta[0], delta[1]);
        }
        break;
      }
#endif
