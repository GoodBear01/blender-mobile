...
static ImBuf *wm_block_splash_image(int *r_width, int *r_height, int max_height)
{
  ImBuf *ibuf = nullptr;
  int width = *r_width;
  int height = 0;
  ...
  if (ibuf) {
    ...
    if (max_height > 0 && height > max_height && ibuf->y > 0) {
      width = max_ii(1, (max_height * ibuf->x) / ibuf->y);
      height = max_height;
    }
...
static ui::Block *wm_block_splash_create(bContext *C, ARegion *region, void * /*arg*/)
{
  ...
  int splash_image_max_h = 0;
  CLAMP_MAX(splash_width, win_size[0] * 0.7f);
#if defined(__ANDROID__)
  const bool portrait = win_size[1] > win_size[0];
  splash_width = int(win_size[0] * (portrait ? 0.94f : 0.52f));
  splash_image_max_h = int(win_size[1] * (portrait ? 0.14f : 0.16f));
#endif
  int splash_height;
  ...
  ImBuf *ibuf = wm_block_splash_image(&splash_width, &splash_height, splash_image_max_h);
...
#if defined(__ANDROID__)
  const bool android_portrait = win_size[1] > win_size[0];
  const int layout_margin_x = int(UI_SCALE_FAC * (android_portrait ? 10 : 16));
  const int layout_height = int(UI_SCALE_FAC * (android_portrait ? 200 : 150));
#else
  const int layout_margin_x = UI_SCALE_FAC * 26;
  const int layout_height = int(UI_SCALE_FAC * 110);
#endif
