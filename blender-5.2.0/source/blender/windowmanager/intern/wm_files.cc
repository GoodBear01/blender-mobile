...
static void wm_gpu_backend_override_from_userdef()
{
  ...
  GPU_backend_type_selection_set_override(GPU_BACKEND_VULKAN);
#else
...
static void wm_init_userdef(Main *bmain)
{
  ...
  /* Older phone userprefs were written with Cycles compiled out. */
...
#ifdef BLENDER_MOBILE
    /* Desktop layouts spawn extra windows and freeze the single Activity surface. */
    UNUSED_VARS(use_prefs);
...
#ifdef BLENDER_MOBILE
    /* Keep the caller path (Download/Documents). Recent desktop paths
     * send the in-app browser to a missing directory and freeze it. */
  {
    char given[FILE_MAX];
    RNA_string_get(op->ptr, "filepath", given);
    if (given[0] == '\0' && blendfile_path && blendfile_path[0]) {
...
{
#ifdef BLENDER_MOBILE
  /* The save-changes popup is easy to miss or get stuck behind on a phone. */
  UNUSED_VARS(C, op, post_action_fn);
  return false;
#else
