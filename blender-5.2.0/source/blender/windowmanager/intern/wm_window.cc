    ...
    case GHOST_kEventWindowMove: {
      ...
      if (state != GHOST_kWindowStateMinimized) {
        ...
        if (wm_window_update_size_position(win)) {
           ...
           * Android needs the same on rotation so landscape fills immediately. */
...
void wm_test_gpu_backend_fallback(bContext *C)
{
  ...
  if (win) {
    ...
    alert(C,
          RPT_("Failed to initialize Vulkan."),
          RPT_("This unofficial phone build requires Vulkan. Update your GPU driver and try again."),
          ui::AlertIcon::Error,
          false);
#else
    alert(C,
