...
#define USER_VERSION_ATLEAST(ver, subver) MAIN_VERSION_FILE_ATLEAST(userdef, ver, subver)
  ...
  userdef->preferences_display_type = USER_TEMP_SPACE_DISPLAY_FULLSCREEN;
  ...
  /* Later versioning may reset these to WINDOW. Keep the single-window host. */
  userdef->filebrowser_display_type = USER_TEMP_SPACE_DISPLAY_FULLSCREEN;
  userdef->preferences_display_type = USER_TEMP_SPACE_DISPLAY_FULLSCREEN;
  userdef->render_display_type = USER_RENDER_DISPLAY_SCREEN;
  userdef->uiflag |= USER_AREA_CORNER_HANDLE;
  userdef->app_flag &= ~(USER_APP_LOCK_EDGE_RESIZE | USER_APP_LOCK_CORNER_SPLIT);
  if (userdef->border_width < 4) {
    userdef->border_width = 5;
  }
#endif
#undef USER_VERSION_ATLEAST
}
